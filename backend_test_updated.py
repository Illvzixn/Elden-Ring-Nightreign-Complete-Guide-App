...
class EldenRingNightReignAPITest(unittest.TestCase):
    ...
    def test_12_search_functionality(self):
        ...
        self.assertIn("builds", data)
        self.assertIn("achievements", data)
        self.assertIn("creatures", data)  # Check for creatures in search results
    ...
    def test_13_filter_bosses(self):
        ...
        response = requests.get(f"{self.base_url}/api/filter-bosses?min_level=10&max_level=15")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertIn("bosses", data)
        print(f"✅ Filter bosses by level range test passed - Found {len(data['bosses'])} bosses in level range 10-15")
    ...
    def test_16_data_validation(self):
        ...
        for boss in data["bosses"]:
            self.assertLessEqual(boss["max_level"], 15, f"Boss {boss['name']} has max_level > 15")
            self.assertGreaterEqual(boss["min_level"], 1, f"Boss {boss['name']} has min_level < 1")
        print("✅ Boss level range validation passed - All bosses have correct level ranges (1-15)")
        ...
        # Check number of walkthroughs (should be 8)
        response = requests.get(f"{self.base_url}/api/walkthroughs")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(len(data["walkthroughs"]), 8, "There should be exactly 8 walkthroughs")
        print("✅ Walkthrough count validation passed - Found exactly 8 walkthroughs")
        
        # Check number of creatures (should be at least 30)
        response = requests.get(f"{self.base_url}/api/creatures")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertGreaterEqual(len(data["creatures"]), 30, "There should be at least 30 creatures")
        print(f"✅ Creature count validation passed - Found {len(data['creatures'])} creatures (>= 30)")
